...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double discountRate = apply_discount((daily_rate(hourly_rate)*22),discount);
    int toReturn = std::ceil(discountRate);
    return toReturn;
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double rate = daily_rate(hourly_rate);
    //calculate the nb of days floored
    double discountedRate = apply_discount(rate, discount);
    int days = std::floor(budget/discountedRate);
    return days;
}